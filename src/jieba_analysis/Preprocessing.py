...
def split_words(file,stopwords):
    ...
    with open(resolve_filename(get_module_res("stockcode2namedict.txt")), 'r', encoding='utf-8') as f:
        stockcode2namedict = eval(f.read())
...
def stockcode2name():
    """
    读取数据库的stock编码和name，放入dict，
    存入文本文件：E:/PyCharm_workplace/stockanalysis/jieba_analysis/stockcode2namedict.txt
    :return:
    """
    # 建立数据库连接
    conn = pymysql.connect("40", "mbi", "zxcvb", "test", charset="utf8")
    # 创建cursor
    cursor = conn.cursor()
    # 拼接参数
    cursor.execute('select distinct substring(symbol,3,6) as symbol,name from spide_gpinfo')
    # 获取所有数据
    data = cursor.fetchall()
    ...
    # 计算stock词频
    for stockcodeandname in data:
        stock2namedict[stockcodeandname[0]]=stockcodeandname[1]

    # 写入文件
    with open(resolve_filename(get_module_res("stockcode2namedict.txt")), 'w', encoding='utf-8') as f:
        f.write(str(stock2namedict))
